...
type RemoveSpecialSignalHandlerFnPtr = unsafe extern "C" fn(c_int, SpecialSignalHandlerFn);

struct Symbols {
    add: AddSpecialSignalHandlerFnPtr,
    #[allow(dead_code)] // exposed for symmetry / future tear-down paths
    remove: RemoveSpecialSignalHandlerFnPtr,
}
...
fn symbols() -> Option<&'static Symbols> {
    ...
    CACHE
        .get_or_init(|| {
            ...
            let add = unsafe { libc::dlsym(libc::RTLD_DEFAULT, c"AddSpecialSignalHandlerFn".as_ptr()) };
            let remove =
                unsafe { libc::dlsym(libc::RTLD_DEFAULT, c"RemoveSpecialSignalHandlerFn".as_ptr()) };
            if add.is_null() || remove.is_null() {
                return None;
            }
            // SAFETY: non-null dlsym results pointing at libsigchain symbols of
            // the documented C signatures.
            Some(Symbols {
                add: unsafe { core::mem::transmute::<*mut c_void, AddSpecialSignalHandlerFnPtr>(add) },
                remove: unsafe {
                    core::mem::transmute::<*mut c_void, RemoveSpecialSignalHandlerFnPtr>(remove)
                },
            })
        })
        .as_ref()
...
/// Returns `true` if `libsigchain.so` is loaded and exports the special-handler
/// API. Useful for diagnostics; `try_register` performs its own lookup.
#[allow(dead_code)]
